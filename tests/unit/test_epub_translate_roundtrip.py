...
def _build_contents_spine_epub(path: Path) -> None:
    with zipfile.ZipFile(path, "w") as zip_file:
        zip_file.writestr("mimetype", "application/epub+zip", compress_type=zipfile.ZIP_STORED)
        zip_file.writestr(
            "META-INF/container.xml",
...
<package xmlns="http://www.idpf.org/2007/opf" version="2.0" unique-identifier="uid">
  ...
  <spine toc="toc"><itemref idref="contents"/></spine>
</package>""",
        )
        zip_file.writestr(
            "contents.xhtml",
            (
                "<html xmlns='http://www.w3.org/1999/xhtml'>"
                "<body><div><a href='chapter1.xhtml'>Chapter One</a></div></body></html>"
            ),
        )
        zip_file.writestr(
            "chapter1.xhtml",
...
def test_translate_roundtrip_skips_toc_like_contents_spine_doc() -> None:
    from ai.epub_translate_roundtrip import run_translate_roundtrip

    with tempfile.TemporaryDirectory() as temp_dir:
        source_epub = Path(temp_dir) / "book-contents.epub"
        output_epub = Path(temp_dir) / "translated-contents.epub"
        _build_contents_spine_epub(source_epub)
        ...
        assert result.translated_segments == 0
        with zipfile.ZipFile(output_epub, "r") as output_zip:
            contents = output_zip.read("contents.xhtml").decode("utf-8")
        assert "Chapter One" in contents
        assert "bw-translation" not in contents
