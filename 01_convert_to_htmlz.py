...
"""
        ...
        if metadata:
            print(
                f"✓ Cached metadata: title={metadata.get('title', 'N/A')}, creator={metadata.get('creator', 'N/A')}"
            )
        return True
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=6000,
        help="Target chunk size in characters (default: 6000)",
    )
        ...
        if os.path.exists(input_html_path):
            ...
            if os.path.exists(config_file):
                try:
                    ...
                    if metadata:
                        print(
                            f"✓ Read cached metadata: title={metadata.get('title', 'N/A')}, creator={metadata.get('creator', 'N/A')}"
                        )
                except Exception as e:
            ...
            if page_files:
                chunk_count = len(
                    [f for f in page_files if not os.path.basename(f).startswith("output_")]
                )
