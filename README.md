...
> Phase 1 已迁移到 Gemini 多模型流程（Flash/Pro/Lite），并支持双语交替输出链路（Step 4 + Step 5）。

这是一个面向长书翻译的文档工具集，支持 PDF/DOCX/EPUB 批量处理与 EPUB 输出。

## ✅ Phase 1 新增能力（BookWeaver）

- Step 3 已切换到 Gemini CLI：`03_translate_md.py`
- 动态模型选择：`ai/model_selector.py`
- 模型适配层：`ai/gemini_provider.py`
- 双语合并：`ai/bilingual_merger.py` + `04_merge_md.py`
- 双语交替 HTML：`05_md_to_html.py`
- 配额追踪：`ai/quota_tracker.py`
- 基准测试：`benchmark_models.py`

### 新 CLI 参数（translatebook.sh）
...
> 说明：下方历史文档仍包含旧版 Claude 描述，后续将继续清理到纯 BookWeaver 版本。
