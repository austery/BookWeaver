# 设计复审 v3 — SPEC-020：Antigravity CLI 迁移与付费 API 授权
...
- 本轮据 `README.md` 与 `pyproject.toml` 确认：EPUB 主入口是 `uv run bookweaver`（`pyproject.toml:10`，`bookweaver = "ai.cli:main"`）；`translatebook.sh` 已归入 README「Legacy Workflow」，仅剩 PDF/DOCX（需 Calibre），SPEC §16 亦自标为 "legacy shell entry point"。
- 据此，v2 的 **IMPROVEMENT 7（shell + Python 双层迁移提示）被降级**：shell 属弃用路径，不再作为正式发现，其仍成立的实现内核已并入「次要观察」。正式发现回到 6 项（编号 1–6，与 v1 一致，内容不变）。
- 两项 BLOCKER 与其余 IMPROVEMENT 均位于 `ai.cli` / `ai.core` / checkpoint / provider 层，**与 shell 入口无关**，不受本次调整影响。
...
在 BLOCKER 1 与 BLOCKER 2 于文档中解决之前，不要将 SPEC-020 晋级为 `🟡 待实施`。发现 3–6 应在同一次修订中处理，但属于澄清而非正确性缺陷。
...
- **§10 对 legacy shell 入口的处置方向值得商榷。** EPUB 主入口已是 `uv run bookweaver`（`pyproject.toml:10`，`ai.cli:main`），`translatebook.sh` 在 README「Legacy Workflow」下仅剩 PDF/DOCX，SPEC §16 亦标其为 "legacy"。但 §10 把工作项写成「更新 translatebook.sh 的 help 与 forwarding」，暗示要让 shell 继续跟进新语义。既然它是弃用中路径，更合适的处置是让其 fallback 转发（`translatebook.sh:762,961`）连同 fallback 概念**一并移除/标废**，而非投入维护。真正需要兑现 §5.3 迁移提示的地方是 **Python `ai.cli` 主入口**：移除 `--cli-api-fallback`（`ai/cli.py:469`）时应**保留参数名并附自定义报错**，否则 `uv run bookweaver … --cli-api-fallback` 会得到 argparse 的 `unrecognized arguments`，而非 §5.3 / Phase 3 承诺的「actionable migration error」。
- **移除范围内的其他遗漏组件。** §10 声称清理过时代码，但未提及：`ai/epub_translate_roundtrip.py` 仍带 `cli_api_fallback_enabled` 接线（`ai/epub_translate_roundtrip.py:671,708`，CLAUDE.md 标为遗留/仅供参考）；`ai/quota_tracker.py`（注释自述「尚未接入主流程」，主树无引用）。建议 §10 对这些给出「清理 / 明确排除范围」的一句话表态。（`ai/model_selector.py` 及配置 `model_thresholds` 当前 EPUB 主流程亦不引用，属遗留 markdown 工作流范畴，可一并按「不关注老路径」处理。）
...
- **§9.4 的 `lite` 复用与 §4 的立论有张力。** §4 拒绝「把 `lite` 映射到 Antigravity Flash Low」，理由是会「制造虚假模型身份与误导性 checkpoint」；而 §9.4 允许用 `--force-resume` 把 `gemini-2.5-flash-lite` 的旧数据当作 `flash` profile 复用。因需显式 `--force-resume`，属知情覆盖、可接受，但建议 §9.4 用一句话点明这是与 §4 一致的「仅在用户显式强制时允许的例外」。
...
- EPUB 主入口为 `uv run bookweaver`（`pyproject.toml:10`，`bookweaver = "ai.cli:main"`）；`translatebook.sh` 已是 README「Legacy Workflow」下的 PDF/DOCX 遗留路径。SPEC §5.1/§8.3 的主命令已正确使用 `uv run bookweaver`。
- 默认模型是具体的 `gemini-2.5-flash`（`ai/cli.py:436`）；SPEC 改为逻辑 `flash` 是真实且有意的默认变更。
- `--cli-api-fallback` 与 `--fallback-provider api` 确实存在并接线了自动付费 fallback（`ai/cli.py:469`、`_CLIAPIFallbackAdapter` 于 `ai/cli.py:625-676`；legacy shell `translatebook.sh:434,763,962`）。§10 的移除范围有据。
...
| 上一版发现 | 当前状态 |
|---|---|
| BLOCKER 1（付费 API 分裂重试）| 保留，未解决（SPEC 未修改）|
| BLOCKER 2（checkpoint 兼容键矛盾）| 保留，未解决 |
| IMPROVEMENT 3（跨代际模型）| 保留，未解决 |
| IMPROVEMENT 4（模型解析子系统处置）| 保留，未解决 |
| IMPROVEMENT 5（§3.4 夸大现状）| 保留，未解决 |
| IMPROVEMENT 6（§9.3 双文件布局）| 保留，未解决 |
| IMPROVEMENT 7（shell + Python 双层迁移提示）| **降级为次要观察**——`translatebook.sh` 属 legacy，仅保留「Python `ai.cli` 入口须保留参数名以出友好报错」的实现提示 |
